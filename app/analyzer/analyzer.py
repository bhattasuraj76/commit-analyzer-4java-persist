...
from . import config
from .core import analyze_test_cases_removal_in_commit_file, analyze_test_cases_addition_in_commit_file
...
"""
  ...
  that test cases added, removed or modified the from java test 
  files 
"""
...
def analyze_test_statistics(repo_url):
    try:
        ...
        for commit in commits:
            ...
            commit_master_data = {"datetime" : commit_datetime, "hash": commit_hash, "msg": commit_msg}
            ...
            for file_idx, file in enumerate(commit.modified_files):
                ...
                removed_test_cases_file = analyze_test_cases_removal_in_commit_file(
                    file)
                added_test_cases_file = analyze_test_cases_addition_in_commit_file(
                    file)
                removed_test_cases_file = {"file_index": file_idx, "filename": filename, **removed_test_cases_file}
                added_test_cases_file = {"file_index": file_idx, "filename": filename, **added_test_cases_file}
            ...
            added_test_function_only = map(lambda x: x["added_test_cases"], all_added_test_cases)
            added_test_function_only = [item for sublist in added_test_function_only for item in sublist]
            ...
            for index, removed_test_cases_data_in_file in enumerate(all_removed_test_cases):
                removed_test_cases_in_file = removed_test_cases_data_in_file["removed_test_cases"]
                for removed_test_case in removed_test_cases_in_file:
                    ...
                    if removed_test_case in added_test_function_only:
                        all_modified_test_cases.append([removed_test_cases_data_in_file["filename"], removed_test_case])
                        temp_all_removed_test_cases[index]["removed_test_cases"].remove(removed_test_case)
                        logger.info(all_removed_test_cases[index], temp_all_added_test_cases)
                        # Remove the false positive from added test cases
                        for add_file_idx, added_test_cases_data_file in enumerate(all_added_test_cases):
                            for added_test_case in added_test_cases_data_file["added_test_cases"]:
                                if removed_test_case == added_test_case:
                                    temp_all_added_test_cases[add_file_idx]["added_test_cases"].remove(removed_test_case)
            ...
            for rm_test_case_file_data_idx, removed_test_cases_in_file_data in enumerate(
                    all_removed_test_cases):
                removed_test_cases_in_file = removed_test_cases_in_file_data["removed_test_cases"]
            ...
            for ad_test_case_file_data_idx, added_test_cases_in_file_data in enumerate(
                    all_added_test_cases):
                removed_test_cases_in_file = added_test_cases_in_file_data["added_test_cases"]
                filename = added_test_cases_in_file_data["filename"]
                for added_test_case_idx_in_file, added_test_case in enumerate(removed_test_cases_in_file):
                    data = {
                        **commit_master_data,
                        "filename": filename,
            ...
            for md_test_case_file_data_idx, modified_test_cases_in_file_data in enumerate(
                    all_modified_test_cases):
